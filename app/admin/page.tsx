...
export default function AdminDashboard() {
    ...
    const deleteLead = async (id: string) => {
        ...
        try {
            const res = await fetch(`/api/admin/leads?id=${id}&password=${encodeURIComponent(password)}`, { method: 'DELETE' });
            if (res.ok) {
        ...
        return (
            <div className="min-h-screen bg-gray-950 flex items-center justify-center p-4">
                <div className="max-w-md w-full bg-white/5 backdrop-blur-xl p-8 rounded-[32px] border border-white/10 shadow-2xl">
                    ...
                    <p className="text-gray-500 text-xs text-center mb-8 uppercase tracking-widest font-bold">Secure Lead Management</p>
            ...
            <div className="max-w-6xl mx-auto">
                ...
                <div className="grid gap-6">
                    {leads.length === 0 ? (
                        ...
                        leads.map((lead) => (
                            <div key={lead.id} className="bg-white p-6 md:p-8 rounded-[32px] border border-gray-100 shadow-sm hover:shadow-xl transition-all group overflow-hidden relative">
